     ...
     mountScrollWorld(document.getElementById('world'), {
       ...
       sections: [
         { id, label, still, poster, posterMobile, clip, clipMobile, accent,
           ...
           artifacts:[{name,description,visibility,href}], // optional clickable exhibit gallery
           hotspots:true, // position artifacts directly over a stable gallery frame
...
function mountScrollWorld(container, config) {
  ...
  if (config.brand) {
    ...
    brand.appendChild(el('span', 'sw-brand__mark logo-sculpt'));
  ...
  const interactlayer = el('div', 'sw-interactlayer');
  const route = el('div', 'sw-route');
  const hint = el('div', 'sw-hint');
  const hintText = el('span'); hintText.textContent = config.hint || 'scroll'; hint.appendChild(hintText);
  hint.appendChild(el('i'));
  const track = el('div', 'sw-track');
  const exhibitDialog = buildExhibitDialog();

  [sky, scrollbar, topbar, stage, copylayer, interactlayer, route, hint, track, exhibitDialog].forEach(n => container.appendChild(n));
  ...
  SECTIONS.forEach((s, i) => {
    ...
    if (s.hotspots) c.classList.add('sw-copy--gallery');
    if (s.featuredExhibits) c.classList.add('sw-copy--featured');
    c.innerHTML =
      ...
      (s.artifacts && s.artifacts.length && !s.hotspots ? `<button class="sw-exhibits-trigger" type="button"><span>Explore ${s.artifacts.length} artifacts</span><i aria-hidden="true">↗</i></button>` : '') +
      (s.cta ? `<div class="sw-copy__cta">${ctaBtns(s.cta)}</div>` : '');
    const trigger = c.querySelector('.sw-exhibits-trigger');
    if (trigger) trigger.addEventListener('click', () => openExhibits(s));

    if (s.hotspots && s.artifacts && s.artifacts.length) {
      const positions = [[8,38],[24,36],[35,39],[45,45],[55,45],[65,39],[77,36],[92,38],[21,53],[33,51],[43,54],[57,54],[68,51],[80,53]];
      const layer = el('div', 'sw-hotspots');
      layer.setAttribute('aria-label', `${s.label || 'Project'} artifacts`);
      s.artifacts.forEach((project, k) => {
        const a = el('a', 'sw-hotspot');
        const pos = positions[k % positions.length];
        a.href = project.href || '#';
        a.style.setProperty('--hx', `${project.x != null ? project.x : pos[0]}%`);
        a.style.setProperty('--hy', `${project.y != null ? project.y : pos[1]}%`);
        a.setAttribute('aria-label', `Open ${project.name}`);
        a.innerHTML = `<span class="sw-hotspot__ring">${String(k + 1).padStart(2, '0')}</span><span class="sw-hotspot__label"><strong>${esc(project.name)}</strong><em>${esc(project.visibility || '')}</em></span>`;
        layer.appendChild(a);
      });
      interactlayer.appendChild(layer);
      s._hotspotLayer = layer;
    }
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      // Preserve the desktop copy block's vertical centring while adding the subtle
      // scroll drift. The previous inline transform replaced translateY(-50%), which
      // could push taller gallery copy below a 720px viewport.
      c.style.transform = (reduce || isMobile() || SECTIONS[i].hotspots) ? 'none' : `translateY(calc(-50% + ${(0.5 - pr) * 4}vh))`;
      c.style.pointerEvents = cop > 0.5 ? 'auto' : 'none';
      if (SECTIONS[i]._hotspotLayer) {
        SECTIONS[i]._hotspotLayer.style.opacity = cop;
        SECTIONS[i]._hotspotLayer.style.pointerEvents = cop > 0.5 ? 'auto' : 'none';
      }
    }
    ...
    const nearSeg = SECTIONS[near]._seg;
    const nearProgress = clamp((y - nearSeg.start) / (nearSeg.end - nearSeg.start), 0, 1);
    container.dispatchEvent(new CustomEvent('scrollworld:progress', {
      detail: { sectionIndex: near, section: SECTIONS[near], progress: nearProgress }
    }));
    if (near !== activeIndex) {
      activeIndex = near;
      ...
      container.classList.toggle('sw-has-hotspots', !!SECTIONS[near].hotspots);
    }
  ...
  function buildExhibitDialog() {
    const dialog = el('dialog', 'sw-exhibits');
    dialog.innerHTML =
      `<div class="sw-exhibits__shell">` +
        `<header class="sw-exhibits__header"><div><span class="sw-exhibits__eyebrow">PROJECT COLLECTION</span><h2 class="sw-exhibits__title"></h2></div><button class="sw-exhibits__close" type="button" aria-label="Close project gallery">×</button></header>` +
        `<p class="sw-exhibits__intro"></p><div class="sw-exhibits__grid"></div>` +
        `<p class="sw-exhibits__note" role="status" aria-live="polite">Select an artifact to open its project page.</p>` +
      `</div>`;
    dialog.querySelector('.sw-exhibits__close').addEventListener('click', () => dialog.close());
    dialog.addEventListener('click', e => { if (e.target === dialog) dialog.close(); });
    return dialog;
  }
  function openExhibits(section) {
    const grid = exhibitDialog.querySelector('.sw-exhibits__grid');
    exhibitDialog.querySelector('.sw-exhibits__title').textContent = section.label || 'Project Gallery';
    exhibitDialog.querySelector('.sw-exhibits__intro').textContent = section.galleryIntro || 'Select an artifact to inspect the project.';
    grid.innerHTML = '';
    section.artifacts.forEach((project, index) => {
      const node = el(project.href ? 'a' : 'button', 'sw-artifact');
      if (project.href) {
        node.href = project.href;
        if (/^https?:/i.test(project.href)) {
          node.target = '_blank';
          node.rel = 'noopener noreferrer';
        }
        node.setAttribute('aria-label', `Open the ${project.name} project page`);
      } else {
        node.type = 'button';
        node.addEventListener('click', () => {
          exhibitDialog.querySelector('.sw-exhibits__note').textContent = `${project.name} is a private or collaborative project. Ask me about the case study.`;
        });
      }
      node.innerHTML =
        `<span class="sw-artifact__index">${String(index + 1).padStart(2, '0')}</span>` +
        `<span class="sw-artifact__body"><strong>${esc(project.name)}</strong><span>${esc(project.description || '')}</span></span>` +
        `<span class="sw-artifact__meta"><em>${esc(project.visibility || (project.href ? 'Public' : 'Private'))}</em><i aria-hidden="true">${project.href ? '↗' : '＋'}</i></span>`;
      grid.appendChild(node);
    });
    exhibitDialog.querySelector('.sw-exhibits__note').textContent = 'Select an artifact to open its project page.';
    if (typeof exhibitDialog.showModal === 'function') exhibitDialog.showModal();
    else exhibitDialog.setAttribute('open', '');
  }
...
function injectCSS() {
  ...
  .sw-root.sw-has-hotspots .sw-copylayer::before{width:min(38vw,520px);opacity:.72}.sw-copy--gallery{top:104px;transform:none!important;width:min(34vw,360px)}.sw-copy--gallery .sw-copy__title{font-size:clamp(1.7rem,3vw,2.6rem)}.sw-copy--gallery .sw-copy__body{font-size:.94rem}.sw-copy--gallery .sw-copy__tags{display:none}
  .sw-copy{position:absolute;left:clamp(18px,5vw,64px);top:50%;transform:translateY(-50%);width:min(42vw,460px);opacity:0;will-change:opacity,transform;}
  .sw-copy--featured{display:none;}
  ...
  .sw-exhibits-trigger{pointer-events:auto;display:flex;align-items:center;justify-content:space-between;gap:18px;width:min(100%,360px);margin-top:22px;padding:14px 17px;border-radius:16px;border:1px solid color-mix(in srgb,var(--sw-accent) 38%,transparent);background:color-mix(in srgb,#fff 68%,transparent);backdrop-filter:blur(12px);color:var(--sw-ink);font:600 .92rem var(--sw-font-body);cursor:pointer;box-shadow:0 14px 36px color-mix(in srgb,var(--sw-ink) 10%,transparent);transition:transform .2s,background .2s;}
  .sw-exhibits-trigger:hover{transform:translateY(-2px);background:color-mix(in srgb,#fff 82%,transparent);}.sw-exhibits-trigger i{font-style:normal;color:var(--sw-accent);}
  .sw-interactlayer{position:fixed;inset:0;z-index:35;pointer-events:none}.sw-hotspots{position:absolute;inset:0;opacity:0;pointer-events:none;transition:opacity .2s}.sw-hotspot{position:absolute;left:var(--hx);top:var(--hy);transform:translate(-50%,-50%);display:grid;place-items:center;text-decoration:none;color:var(--sw-ink);pointer-events:auto}.sw-hotspot__ring{display:grid;place-items:center;width:34px;height:34px;border-radius:50%;font:700 .62rem ui-monospace,Menlo,monospace;color:#fff;background:color-mix(in srgb,var(--sw-accent) 88%,#1e1b24);border:2px solid rgba(255,255,255,.88);box-shadow:0 0 0 6px color-mix(in srgb,var(--sw-accent) 22%,transparent),0 10px 24px rgba(37,39,56,.22);transition:transform .2s,box-shadow .2s}.sw-hotspot:hover .sw-hotspot__ring,.sw-hotspot:focus-visible .sw-hotspot__ring{transform:scale(1.18);box-shadow:0 0 0 9px color-mix(in srgb,var(--sw-accent) 25%,transparent),0 12px 30px rgba(37,39,56,.28)}.sw-hotspot__label{position:absolute;left:50%;top:44px;transform:translateX(-50%) translateY(4px);display:flex;flex-direction:column;gap:3px;width:max-content;max-width:180px;padding:8px 11px;border-radius:11px;background:rgba(255,255,255,.92);border:1px solid color-mix(in srgb,var(--sw-accent) 25%,transparent);box-shadow:0 12px 28px rgba(37,39,56,.16);opacity:0;pointer-events:none;transition:opacity .18s,transform .18s;text-align:center}.sw-hotspot__label strong{font:700 .78rem var(--sw-font-display)}.sw-hotspot__label em{font-style:normal;font-size:.58rem;letter-spacing:.09em;text-transform:uppercase;color:var(--sw-accent)}.sw-hotspot:hover .sw-hotspot__label,.sw-hotspot:focus-visible .sw-hotspot__label{opacity:1;transform:translateX(-50%) translateY(0)}
  ...
  .sw-route{position:fixed;right:clamp(14px,2.4vw,30px);top:50%;z-index:70;transform:translateY(-50%);display:flex;flex-direction:column;gap:22px;padding:18px 10px;}
  ...
  .sw-exhibits{position:fixed;inset:0;margin:auto;width:min(1060px,calc(100vw - 36px));max-height:min(82vh,820px);padding:0;border:0;border-radius:28px;background:color-mix(in srgb,var(--sw-bg) 94%,#fff);color:var(--sw-ink);box-shadow:0 32px 100px rgba(34,26,29,.28);z-index:100;overflow:hidden;}
  .sw-exhibits::backdrop{background:rgba(25,19,21,.45);backdrop-filter:blur(8px);}.sw-exhibits__shell{padding:clamp(22px,4vw,44px);max-height:min(82vh,820px);overflow:auto;box-sizing:border-box;}
  .sw-exhibits__header{display:flex;align-items:flex-start;justify-content:space-between;gap:24px}.sw-exhibits__eyebrow{display:block;font-size:.72rem;letter-spacing:.18em;font-weight:800;color:var(--sw-accent)}.sw-exhibits__title{margin:8px 0 0;font:700 clamp(1.8rem,4vw,3.2rem)/1 var(--sw-font-display)}
  .sw-exhibits__close{border:1px solid color-mix(in srgb,var(--sw-ink) 18%,transparent);background:transparent;color:var(--sw-ink);width:42px;height:42px;border-radius:50%;font-size:1.7rem;line-height:1;cursor:pointer}.sw-exhibits__intro{color:var(--sw-ink-soft);max-width:62ch;line-height:1.55;margin:18px 0 26px}
  .sw-exhibits__grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.sw-artifact{appearance:none;text-align:left;text-decoration:none;display:grid;grid-template-columns:36px 1fr auto;gap:14px;align-items:start;min-height:116px;padding:18px;border:1px solid color-mix(in srgb,var(--sw-accent) 22%,transparent);border-radius:18px;background:color-mix(in srgb,#fff 56%,transparent);color:var(--sw-ink);font-family:var(--sw-font-body);cursor:pointer;transition:transform .2s,border-color .2s,background .2s}.sw-artifact:hover{transform:translateY(-2px);border-color:var(--sw-accent);background:#fff}.sw-artifact__index{font:600 .72rem ui-monospace,Menlo,monospace;color:var(--sw-accent);padding-top:3px}.sw-artifact__body{display:flex;flex-direction:column;gap:7px}.sw-artifact__body strong{font:700 1.05rem var(--sw-font-display)}.sw-artifact__body>span{font-size:.85rem;line-height:1.45;color:var(--sw-ink-soft)}.sw-artifact__meta{display:flex;align-items:center;gap:8px}.sw-artifact__meta em{font-style:normal;font-size:.68rem;letter-spacing:.08em;text-transform:uppercase;color:var(--sw-accent)}.sw-artifact__meta i{font-style:normal}.sw-exhibits__note{font-size:.78rem;color:var(--sw-ink-soft);margin:20px 0 0}
  @media (max-width:860px){
    ...
    .sw-exhibits__grid{grid-template-columns:1fr}.sw-exhibits{width:calc(100vw - 20px);max-height:88vh;border-radius:22px}.sw-exhibits__shell{max-height:88vh;padding:22px}.sw-artifact{grid-template-columns:30px 1fr;min-height:auto}.sw-artifact__meta{grid-column:2}.sw-exhibits-trigger{width:100%;}
    .sw-hotspots{left:0;right:0;top:auto;bottom:calc(18px + env(safe-area-inset-bottom));height:auto;padding:10px 48px 10px 14px;display:flex;gap:9px;overflow-x:auto;scroll-snap-type:x proximity;background:linear-gradient(0deg,color-mix(in srgb,var(--sw-bg) 86%,transparent),transparent);}.sw-hotspot{position:relative;left:auto;top:auto;transform:none;flex:0 0 auto;scroll-snap-align:start}.sw-hotspot__ring{width:40px;height:40px}.sw-hotspot__label{top:auto;bottom:50px}.sw-copy--gallery{top:auto;bottom:120px;left:18px;right:18px;width:auto;max-width:420px}.sw-copy--gallery .sw-copy__body{display:none}
